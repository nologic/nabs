...
void init_featureset(struct payload_featureset * f, unsigned int size)
{

	memset(f, 0, sizeof(struct payload_featureset));
	...
	f->fftout = fftw_malloc(f->fftout_size * sizeof(fftw_complex));
	f->plan = fftw_plan_dft_r2c_1d(size, f->fftin, f->fftout, FFTW_MEASURE);
}

void finit_featureset(struct payload_featureset * f)
{
	free(f->fftin);
	free(f->histogram);
	fftw_free(f->fftout);
...
void compute_features(struct payload_featureset *f, const unsigned char *data, unsigned int size)
{
