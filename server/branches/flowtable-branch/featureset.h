#ifndef INCLUDED_FEATURESET_H
#define INCLUDED_FEATURESET_H
...
void init_featureset(struct payload_featureset *, unsigned int);
void finit_featureset(struct payload_featureset *);
void compute_features(struct payload_featureset *, const unsigned char *, unsigned int);

static inline void normalize_features(struct payload_featureset * f, unsigned int n)
{
	f->mean = f->mean / n;
...
static inline void init_svmnode(const struct payload_featureset * f, struct svm_node *x)
{
...
#if 0 /* keeping it for future record only! */
