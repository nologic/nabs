...
static inline int init_flow_content_stats(struct flow_content *fc, struct sb_config *sbc, struct eu_packet *ep){
	...
	if(get_chunk_from_buffer(sbc, (void *)&fc->fr, sizeof(struct flow_record), &fc->sb) != 0){
		fprintf(stderr, "could not allocate memory from send buffers!\n");
		fprintf(stderr, "try increasing number of send buffers...\n");
		return ERROR;
	}
...
int flowtable_init(struct flow_table *table, unsigned int buckets, struct sb_config *sb){
	unsigned int i;
	void* tmp;

	if((table == NULL) || (sb == NULL) || (buckets == 0))
		return -1;
...
int flowtable_add_flow(struct flow_table *ft, struct eu_packet *e)
{
	...
	if (list_empty(&(ft->freelist.list))) {
		...
		if(init_flow_content_stats(tmp, ft->sb, e) == ERROR)
			return ERROR;
		buffer_packet_payload(tmp, e);
