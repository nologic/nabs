...
public class ReceptorComm implements Runnable {
    ...
    private void procAnalysisSummaryMessage(AnalysisSummaryMessage msg) {
        receptor.getManager().setDatabaseReport(msg);
    }

    private void procAnalysisReportMessage(AnalysisReportMessage msg) {
        ModuleHandle handle = msg.getHandle();
        AnlzFrontendModule mod = (AnlzFrontendModule)receptor.getManager().getModule(handle);
        
        mod.setResult(msg.getReportInputStream());
        receptor.getManager().moduleUpdated(mod);
    }
    ...
    private void procModuleStatusMessage(ModuleStatusMessage msg) throws IOException {
        ...
        ProcFrontendModule mod = (ProcFrontendModule)receptor.getManager().getModule(handle);
    ...
    private void procModuleInterCommMessage(ModuleInterCommMessage msg) throws IOException {
        ...
        ProcFrontendModule mod = (ProcFrontendModule) receptor.getManager().getModule(msg.getModuleHandle());
