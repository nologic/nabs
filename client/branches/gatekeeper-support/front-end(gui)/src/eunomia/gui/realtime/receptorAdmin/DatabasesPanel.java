...
public class DatabasesPanel extends JPanel implements ListSelectionListener, ActionListener, ReceptorStateListener {
    ...
    public DatabasesPanel(Receptor rec) {
        ...
        dbPanel.setDatabase(null, false);
    }
    ...
    public void update(){
        ...
        if(oldDb == null){
            if(array.length > 0){
                dbPanel.setDatabase((DatabaseDescriptor)array[0], false);
            }
        } else {
            dbPanel.setDatabase(receptor.getState().getDatabaseDescriptor(oldDb.getName()), false);
        }
    }
