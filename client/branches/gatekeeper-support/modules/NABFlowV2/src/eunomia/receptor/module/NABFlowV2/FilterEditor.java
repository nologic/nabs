...
public class FilterEditor extends JPanel implements FilterEntryEditor {
     ...
     * Entry to be edited.
     */
    private NABFilterEntry nabEntry;
    ...
    private void addControls(){
        ...
        types = new JCheckBox[NABFlowV2.NUM_TYPES];
        for (int i = 0; i < types.length; i++) {
            add(types[i] = new JCheckBox(NABFlowV2.typeNames[i]));
        }
    }
    ...
    public void setFilterEntry(FilterEntry entry) {
        ...
        // still be performed.
        if(entry instanceof NABFilterEntry){
            setEnabled(true);
            
            // The entry has the list of selected types. Transfer that to the check boxes.
            nabEntry = (NABFilterEntry)entry;
            for (int i = 0; i < types.length; i++) {
                types[i].setSelected(nabEntry.isAllowed(i));
            }
        } else {
    ...
    public void commitChanges() {
        ...
        for (int i = 0; i < types.length; i++) {
            nabEntry.setAllowed(i, types[i].isSelected());
        }
    }
}