...
public class ModuleManager implements Runnable, ConfigChangeListener, ModuleLinkerListener, ConsoleModuleManager {
    private static String[] classPrefix = new String[] {
        "eunomia.module.frontend.proc.", // PROC
        "eunomia.module.receptor.flow.", //FLOW
        "eunomia.module.frontend.anlz." // ANLZ
    };
    
    private static String[] classPrefix_dep = new String[] {
    ...
    private EunomiaModule newModuleInstance(ModuleDescriptor desc) throws ClassNotFoundException, InstantiationException, IllegalAccessException, NoModuleJarException{
        ...
        className = classPrefix[desc.getType()] + desc.getName() + ".Main";
        try {
            klass = loader.loadClass(className);
        } catch (ClassNotFoundException ex) {
            className = classPrefix_dep[desc.getType()] + desc.getName() + ".Main";
            klass = loader.loadClass(className);
        }
    ...
    private FrontendModule instantiateModule(ModuleHandle handle) throws ClassNotFoundException, InstantiationException, IllegalAccessException, NoModuleJarException, InvocationTargetException {
        ModuleDescriptor desc = receptor.getLinker().getMapping(handle.getModuleName(), handle.getModuleType());
        if(desc == null){
            throw new NoModuleJarException("Loading module: No JAR specified for module " + handle);
        }

        EunomiaModule module = newModuleInstance(desc);
    ...
    public void updateModule(FrontendModule module){
        ...
        if(isNotUpdating){
            if(module instanceof ProcFrontendModule || module instanceof AnlzFrontendModule) {
    ...
    public ModuleHandle getModuleHandle(FrontendProcessorModule mod){
