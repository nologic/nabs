 ...
 *
 * $Id: FileProcessor.java,v 1.17.2.10 2008/01/07 15:14:08 cwl Exp $
 */
...
class FileProcessor extends DaemonThread {
     ...
     * processing.
     */
    private void noteDbsRequiringSync(DatabaseImpl db, Set deferredWriteDbs) {
        
        /*
         * We add all non-deleted DBs to the set, not just DeferredWrite DBs,
         * because any DB may be opened as DeferredWrite at a future time.  If
         * is it opened as DeferredWrite between now and the time the
         * checkpointer runs, we must be sure to flush it during the
         * checkpoint.  For example, if the DB is not currently open by a user
         * but is being cleaned, it will not be in DeferredWrite mode.  But a
         * user may open it as DeferredWrite at any time, and its mode will
         * change to DeferredWrite.  [#15913]
         */
        if (db != null && !db.isDeleted()) {
