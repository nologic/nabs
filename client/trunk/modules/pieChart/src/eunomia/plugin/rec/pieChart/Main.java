...
public class Main implements FlowProcessor, ReceptorProcessorModule {
    ...
    public void newFlow(Flow flow) {
        ...
        if(doProc){
            ...
            if(flow instanceof NABFlow){
                ...
                int type = nFlow.getType();
                
                ++percents[type];
                counts[type] += nFlow.getSize();
            } else if(flow instanceof NABFlowV2) {
                NABFlowV2 nFlow = (NABFlowV2)flow;

                int size = nFlow.getSize();
                
                for (int i = 0; i < NABFlowV2.NUM_TYPES; i++) {
                    int count = nFlow.getTypeCount(i);
                    percents[i] += count;
                    counts[i] += count * nFlow.getSize();
                }
            }
        }
    }
    ...
    public boolean accept(FlowModule module) {
        // at this point the module agrees to accept selected flow types.
