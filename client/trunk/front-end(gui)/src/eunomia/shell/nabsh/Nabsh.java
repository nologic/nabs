...
public class Nabsh extends JPanel implements ActionListener, Runnable, ConsoleInterface, KeyListener, Exiter, Shell {
    ...
    private static char[] SEP_CHAR_LIST = ";.~!@#$%^&*()_+{}[]'\",\\| ".toCharArray();
    ...
    private void addControls() {
        ...
        entry.setFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS, Collections.EMPTY_SET);
    }
    ...
    private void tabComplete() {
        ...
        for (int i = text.length - 1; i != -1; --i) {
            for (int j = 0; j < SEP_CHAR_LIST.length; j++) {
                if(text[i] == SEP_CHAR_LIST[j]) {
                    lastIndex = i;
                    break;
                }
            }
