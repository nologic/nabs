...
public class ConversationList {
    ...
    public void writeOut(DataOutputStream dout) throws IOException {
        Conversation[] convs = getArray();
        dout.writeInt(convs.length);
        for(int i = 0; i < convs.length; i++){
            convs[i].writeOut(dout);
        }
    }
    ...
    public void readIn(DataInputStream din) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            readConv.readIn(din);
            Conversation existing = findConversation(readConv.getRmt_ip(), readConv.getLcl_port(), readConv.getRmt_port());
