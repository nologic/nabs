...
public class Host extends JPanel implements ConfigChangeListener,
    ...
    public Host(long ip, InetAddress hostAddr, GUIModule mod) {
        module = mod;
        host = hostAddr;
        firstUpdate = true;
    ...
    private void configurePopupMenu(){
        ...
        ModuleManager mmon = module.getReceptor().getManager();
        java.util.List hvList = mmon.getModuleList("hostDetails");
        if(hvList.size() > 0){
            Iterator it = hvList.iterator();
            while (it.hasNext()) {
                GUIModule mod = (GUIModule) it.next();
                ModuleHandle hand = mmon.getModuleHandle(mod);
