...
public class ConnectionsTable extends JPanel implements TableModel, MouseListener {
    private static final String[] colNames = new String[]{"Last Seen", "Local Port",
                "Remote Port", "Remote IP", "Remote Host Name", "Total", "Ingress", "Egress"};
    ...
    public void updateConversations(Conversation[] c){
        if(comparator != null){
            //Do we need to use array reuse here?
            convs = new Conversation[c.length];
            System.arraycopy(c, 0, convs, 0, c.length);
            Arrays.sort(convs, comparator); // this uses mergesort.
        } else {
            convs = c;
        }
    ...
    public Object getValueAt(int r, int columnIndex) {
        switch(columnIndex){
            case 0:
                ...
                cal.setTimeInMillis(convs[r].getLastActive());
                return  (h < 10?"0":"") + h + ":" + (m < 10?"0":"") + m + ":" + (s < 10?"0":"") + s;
            case 1: return convs[r].getLclPort();
            case 2: return convs[r].getRmtPort();
            case 3: return convs[r].getRmtAddress().getHostAddress();
            case 4: return convs[r].getRmtHost();
            case 5: return Util.convertBytes(convs[r].getTotalBytes());
            case 6: return Util.convertBytes(convs[r].getInBytes());
            case 7: return Util.convertBytes(convs[r].getOutBytes());
        }
    ...
    private void addControls(){
        ...
        model.getColumn(5).setPreferredWidth(15);
